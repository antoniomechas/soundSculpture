...
void ShadowMapLight::setup( int shadowMapSize, float fov, float nearr, float farr ) {

    // make a projection matrix that we'll use to render a scene from our light's viewpoint
    m_projectionMatrix.makePerspectiveMatrix(fov, shadowMapSize/shadowMapSize, nearr, farr);
    ...
    m_linearDepthScalar = 1.0 / (farr - nearr); // this helps us remap depth values to be linear
