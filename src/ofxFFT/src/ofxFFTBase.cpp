...
void ofxFFTBase::initFFT() {
	...
	fftSubbands = new float[OFX_FFT_SUBBANDS];
	fftSubbandsPeak = new float[OFX_FFT_SUBBANDS];
}
...
void ofxFFTBase::killFFT() {
	...
	if(fftSubbandsPeak) {
        delete[] fftSubbandsPeak;
        fftSubbandsPeak = NULL;
    }
...
void ofxFFTBase::updateAudioData(ofxFFTData & audioData, float * dataNew) {
	...
	int fft_bins = audioData.size;
	
	for(int i = 0; i < OFX_FFT_SUBBANDS; i++)
    {
        fftSubbands[i] = 0;
        for(int b = 0; b < fft_bins/OFX_FFT_SUBBANDS; b++)
        {
            fftSubbands[i] +=  audioData.data[i*(fft_bins/OFX_FFT_SUBBANDS)+b];
        }
        fftSubbands[i] = fftSubbands[i]*(float)OFX_FFT_SUBBANDS/(float)fft_bins;
    }
	...
	for(int i = 0; i < OFX_FFT_SUBBANDS; i++)
    {
        ...
        for(int b = 0; b < fft_bins/OFX_FFT_SUBBANDS; b++)
        {
            fftSubbandsPeak[i] +=  audioData.dataPeak[i*(fft_bins/OFX_FFT_SUBBANDS)+b];
        }
        fftSubbandsPeak[i] = fftSubbandsPeak[i]*(float)OFX_FFT_SUBBANDS/(float)fft_bins;
    }
...
//    drawFftData(audioData, width, height);   // this is audio data before its been normalised, good for debugging.
	...
	drawSubBands(audioData, width,height);
...
void ofxFFTBase::drawSubBands(const ofxFFTData & audioData, int w, int h) {
    int bx, by; // border.
    bx = by = renderBorder;
    ...
    ofSetColor(0,255,0);
    for(int i=0; i<OFX_FFT_SUBBANDS; i++) {
        ofRect(i * renderSingleBandWidth * (audioData.size / OFX_FFT_SUBBANDS) + bx,
               h + by,
               renderSingleBandWidth * (audioData.size / OFX_FFT_SUBBANDS),
               -fftSubbands[i] * h);
    }
    ofSetColor(255,0,0);
    for(int i=0; i<OFX_FFT_SUBBANDS; i++) {
